...
import { Constants, translate } from "@azure/amqp-common";
...
export class BatchingReceiver extends MessageReceiver {
   ...
   * encountering an error on the AMQP link, session or connection.
   * Will be set to undefined if there is no ongoing receive request.
   * @param error The error on the AMQP link, session or connection.
   * @param causedByDisconnect Whether this method is called as a result of a connection disconnect.
   */
  private _finalErrorHandler:
    | ((error: AmqpError | Error, causedByDisconnect?: boolean) => void)
    | undefined;
  ...
  async onDetached(connectionError?: AmqpError | Error): Promise<void> {
    ...
    if (typeof this._finalErrorHandler !== "function") {
    ...
    this._finalErrorHandler(connectionError, causedByDisconnect);
  }
  ...
  async receive(
    ...
    try {
      const result = await this._receiveImpl(maxMessageCount, maxWaitTimeInSeconds);
      log.batching(
      ...
      this._finalErrorHandler = undefined;
    }
  }
  ...
  private _receiveImpl(
    ...
    return new Promise<ServiceBusMessage[]>((resolve, reject) => {
      ...
      const onSessionError: OnAmqpEvent = (context: EventContext) => {
        ...
        log.error(
          "[%s] 'session_error' event occurred for Receiver '%s' with error:\n%O",
          this._context.namespace.connectionId,
          this.name,
          sessionError
        );
        ...
        if (typeof this._finalErrorHandler === "function") {
          const error =
            sessionError ||
            new Error("Unknown error occurred on the AMQP session while receiving messages.");
          this._finalErrorHandler(error);
        }
      };

      this._finalErrorHandler = (error: AmqpError | Error, causedByDisconnect?: boolean): void => {
        if (totalWaitTimer) {
        ...
        // If connection was disconnected, then we either return the collected messages if in
        // ReceiveAndDelete mode (because otherwise they are lost forever) or bubble up the error
        if (causedByDisconnect) {
          if (this.receiveMode === ReceiveMode.receiveAndDelete && brokeredMessages.length) {
            return resolve(brokeredMessages);
          }
          return reject(translate(error));
        }
      };
      ...
      const onReceiveMessage: OnAmqpEventAsPromise = async (context: EventContext) => {
        ...
        try {
          ...
          if (typeof this._finalErrorHandler === "function") {
            return this._finalErrorHandler(errObj);
          }
        }
        if (brokeredMessages.length === maxMessageCount) {
          finalAction();
      ...
      const onReceiveError: OnAmqpEvent = (context: EventContext) => {
        const receiverError = context.receiver && context.receiver.error;
        log.error(
          "[%s] 'receiver_error' event occurred for Receiver '%s' with error:\n%O",
          this._context.namespace.connectionId,
          this.name,
          receiverError
        );
        if (typeof this._finalErrorHandler === "function") {
          const error =
            receiverError ||
            new Error("Unknown error occurred on the AMQP link while receiving messages.");
          this._finalErrorHandler(error);
        }
      };
